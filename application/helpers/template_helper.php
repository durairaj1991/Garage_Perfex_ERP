...
function get_company_logo($uri = '', $href_class = '', $type = '')
{
    ...
    if ($company_logo != '') {
        ...
        <img src="' . base_url('uploads/company/' . $company_logo) . '" class="img-responsive newbie" alt="' . e($company_name) . '">
        </a>';
    } elseif ($company_name != '') {
...
function override_merge_fields(array $mergeFields, ?string $content): ?string
{
    ...
    foreach ($mergeFields as $key => $val) {
        ...
        $replacement = sprintf(
            '<span data-merge-field="%s">%s</span>',
            trim($key, "{}"),
            $replacement
        );
        $content = str_ireplace($key, $replacement, $content);
    }
    return $content;
}
